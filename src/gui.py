...
class GUIAlignment:
	x: float
	y: float
	type: GUIAlignmentType

	def __init__(self, x: float, y: float, type: GUIAlignmentType):
		self.x = x
		self.y = y
		self.type = type
	...
	def get_coords(self, dst: pygame.Surface, src: pygame.Surface):
		...
		if self.type & GUIAlignmentType.CENTER_X:
			x = dst.get_width() / 2 - src.get_width() / 2

		if self.type & GUIAlignmentType.CENTER_Y:
			y = dst.get_height() / 2 - src.get_height() / 2

		if self.type & GUIAlignmentType.RIGHT:
			x = dst.get_width() - src.get_width()

		if self.type & GUIAlignmentType.BOTTOM:
